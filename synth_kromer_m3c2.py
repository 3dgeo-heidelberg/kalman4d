...
def proc_cp(p_changes, p_unc, p_dates, corepoint_id, filter_size):
    ...
    ts_raw = np.array(p_dates)[valid_idx]  # time series in days
    ts_raw -= ts_raw[0]
    f = scipy.interpolate.interp1d(ts_raw, p_changes, fill_value="extrapolate")
    ts = np.arange(ts_raw[0], ts_raw[-1], 3600)
    changes = f(ts)
...
#    return None
    ...
    if exportFilter:
        ...
        all_dates = sorted(np.unique(all_dates))
...
if __name__ == '__main__':
    ...
    outfile = fr"synth_kalman\kromer_w{filter_size}.las"
    # outfile = fr"synth_kalman\interpolated.las"
